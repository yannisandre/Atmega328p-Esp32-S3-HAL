...
FEATURE := $(FEATURE_ESP) # à changer par $(FEATURE_ESP) pour l'ESP32-S3 ou par $(FEATURE_ATMEGA) pour l'ATMEGA

TARGET = $(ESP32_TARGET) # à changer par $(ESP32_TARGET) pour l'ESP32-S3 ou par $(ATMEGA_TARGET) pour l'ATMEGA
...
UPLOAD_CMD = $(ESPTOOL_CMD) # à changer par $(ESPTOOL_CMD) pour l'ESP32-S3 ou par $(AVRDUDE_CMD) pour l'ATMEGA
...
COMPILE_CMD = $(ESPCOMP_CMD) # à changer par $(ESPCOMP_CMD) pour l'ESP32-S3 ou par $(ATMEGACOMP_CMD) pour l'ATMEGA
